...
static void draw_food(SnakeGamestate *gs) {
  lcd_fillRect(gs->food_x * 4 + 1, gs->food_y * 4 + 1, gs->food_x * 4 + 2,
               gs->food_y * 4 + 2, 1);
}
...
static void set_snake_direction(SnakeGamestate *gs, uint16_t i,
                                Direction direction) {
  gs->direction_circ_buffer[i / 4] &=
      ~(0b00000011 << (i % 4) * 2);  // set to zero
  gs->direction_circ_buffer[i / 4] |= direction
                                      << (i % 4) * 2;  // set to direction
}
...
static void reset_snake(SnakeGamestate *gs) {
  last_pressed_keys = (struct pressed_keys){0};
  gs->length = 1;
  gs->head_x = 0;
  gs->head_y = 0;
...
void run_snake() {
  ...
  while (1) {
    ...
    enum AppKey last_key = read_last_key();
    if (read_key() == APP_KEY_EXIT) {
      return;
    } else if (last_key == APP_KEY_RIGHT && snake_state.snake_dir != LEFT) {
      snake_state.snake_dir = RIGHT;
    } else if (last_key == APP_KEY_LEFT && snake_state.snake_dir != RIGHT) {
      snake_state.snake_dir = LEFT;
    } else if (last_key == APP_KEY_UP && snake_state.snake_dir != DOWN) {
      snake_state.snake_dir = UP;
    } else if (last_key == APP_KEY_DOWN && snake_state.snake_dir != UP) {
    ...
    } else if (lcd_check_buffer(snake_state.head_x * 4,
                                snake_state.head_y * 4)) {
