...
static void draw_terrain(struct Terrain *terrain) {
    lcd_drawLine(0, DISPLAY_HEIGHT - 1, landing_pad_width, DISPLAY_HEIGHT - 1,
                 1);
...
static bool lander_terrain_collision(struct LanderGamestate *s) {
    if (s->lander.pos.x < -14 || s->lander.pos.x >= DISPLAY_WIDTH + 14 ||
        ...
        return true;
    return (point_terrain_collision(add(s->lander.t1p1, s->lander.pos),
                                    &s->terrain) ||
            point_terrain_collision(add(s->lander.t1p2, s->lander.pos),
                                    &s->terrain) ||
            point_terrain_collision(add(s->lander.t1p3, s->lander.pos),
                                    &s->terrain));
}
