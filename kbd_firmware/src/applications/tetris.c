...
#define NUM_PIECES 7
enum PieceType { P_I, P_J, P_L, P_O, P_S, P_T, P_Z };
...
// X . . .
// X X X .
static const uint8_t block_shapes[] = {0b11110000, 0b10001110, 0b00101110,
                                       0b01100110, 0b01101100, 0b01001110,
                                       0b11000110};
...
bool check_block(uint8_t x, uint8_t y) {
  return lcd_check_buffer(X_OFFSET + x * BLOCK_DIM, Y_OFFSET + y * BLOCK_DIM);
}
...
void draw_block(uint8_t x, uint8_t y, struct TetrisGamestate *state) {
  ...
  lcd_fillRect(X_OFFSET + x * BLOCK_DIM, Y_OFFSET + y * BLOCK_DIM,
               X_OFFSET + (x + 1) * BLOCK_DIM - 1,
               Y_OFFSET + (y + 1) * BLOCK_DIM - 1, state->flag);
}

void collide_block(uint8_t x, uint8_t y, struct TetrisGamestate *state) {
  // we need to check for side wall collision even if block is outside the play
  // area otherwise it can be moved into an illegal position before it is fully
  // inside
  if (x >=
      BOARD_WIDTH) {  // don't need to check other direction due to wraparound
    state->flag = 1;
    return;
  }
  if ((y >= BOARD_HEIGHT &&
       y < 255 - 4)  // don't show collision for blocks that enter the play area
      || check_block(x, y)) {
    state->flag = 1;
  }
}
...
void apply_to_piece_blocks(void (*f)(uint8_t, uint8_t,
                                     struct TetrisGamestate *),
...
void rotate_piece(struct TetrisGamestate *state) {
  ...
  for (uint8_t offset = 0; offset <= 2; offset++) {
    for (int8_t offset_direction = -1; offset_direction <= 1;
         offset_direction += 2) {
...
__attribute__((optimize("s"))) void check_and_handle_full_lines(
    struct TetrisGamestate *state) {
...
void draw_game_area() {
  lcd_drawRect(X_OFFSET - 1, Y_OFFSET - 1, X_OFFSET + BLOCK_DIM * BOARD_WIDTH,
...
void draw_next_piece(struct TetrisGamestate *state) {
  lcd_fillRect(X_OFFSET + BOARD_WIDTH * BLOCK_DIM + BLOCK_DIM,
               Y_OFFSET + 2 * BLOCK_DIM,
               X_OFFSET + BOARD_WIDTH * BLOCK_DIM + 10 * BLOCK_DIM,
               8 + 7 * BLOCK_DIM, 0);
