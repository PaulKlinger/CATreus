...
enum MinesGamePhase { m_FIRSTMOVE, m_GAME, m_END };
 ...
 */
static const uint8_t digit_font[] = {
    0b00000000, 0b01100101, 0b11110010, 0b01111101, 0b11111011, 0b11001011,
    0b01011010, 0b01111111, 0b11001001, 0b1111111,  0b11111110, 0b01000000};

// static const uint8_t digit_font[] = {0b11110111,0b11100101, 0b11110010,
// 0b01111101, 0b11111011, 0b11001011, 0b01011010, 0b01111111, 0b11001001,
// 0b1111111, 0b11111110, 0b01000000};
...
static void draw_digit(uint8_t digit, uint8_t cell_x, uint8_t cell_y) {
    ...
    for (uint8_t dy = 0; dy < 3; dy++) {
        for (uint8_t dx = 0; dx < 3; dx++) {
            ...
            lcd_drawPixel(
                x + dx, y + dy,
                !!(digit_font[bit_index / 8] & (1 << (7 - bit_index % 8))));
        }
    }
}

static void draw_hidden_cell(uint8_t cell_x, uint8_t cell_y) {
    lcd_drawPixel(3 + x_cell(cell_x), 3 + y_cell(cell_y), 1);
}

static void draw_mine(uint8_t cell_x, uint8_t cell_y) {
    // draw a cross for mines and flags
    for (uint8_t i = 0; i < 5; i++) {
        lcd_drawPixel(x_cell(cell_x) + 1 + i, y_cell(cell_y) + 1 + i, 1);
        lcd_drawPixel(x_cell(cell_x) + 1 + i, y_cell(cell_y) + 5 - i, 1);
    }
}

static void draw_grid() {
    for (uint8_t cell_x = 0; cell_x <= GRID_X; cell_x++) {
        lcd_drawLine(x_cell(cell_x), Y_OFFSET, x_cell(cell_x), y_cell(GRID_Y),
                     1);
    }
    for (uint8_t cell_y = 0; cell_y <= GRID_Y; cell_y++) {
        lcd_drawLine(X_OFFSET, y_cell(cell_y), x_cell(GRID_X), y_cell(cell_y),
                     1);
    }
}

static void draw_cursor(struct MinesGameState *state) {
    lcd_drawRect(x_cell(state->cursor_x) + 1, y_cell(state->cursor_y) + 1,
                 x_cell(state->cursor_x) + 5, y_cell(state->cursor_y) + 5, 1);
}

// noinline to save memory
__attribute__((noinline)) static uint8_t mines_count(
    struct MinesGameState *state, uint8_t cell_x, uint8_t cell_y) {
    uint8_t nmines = 0;
    for (int8_t dy = -1; dy <= 1; dy++) {
        for (int8_t dx = -1; dx <= 1; dx++) {
            if (cell_x + dx >= GRID_X || cell_x + dx < 0 ||
                cell_y + dy >= GRID_Y || cell_y + dy < 0)
                continue;
            nmines += bitmatrix_get(state->mines, cell_x + dx, cell_y + dy);
        }
    }
    return nmines;
}
...
static void draw_remaining_mine_count(struct MinesGameState *state) {
    char mine_count_str[8];
    sprintf(mine_count_str, "%d", NMINES - state->n_flagged);
    lcd_gotoxy(0, 1);
...
static void show_game_state(struct MinesGameState *state) {
    ...
    for (uint8_t cell_y = 0; cell_y < GRID_Y; cell_y++) {
        for (uint8_t cell_x = 0; cell_x < GRID_X; cell_x++) {
            if ((state->game_phase != m_END &&
                 bitmatrix_get(state->flagged, cell_x, cell_y)) ||
                (state->game_phase == m_END &&
                 bitmatrix_get(state->mines, cell_x, cell_y))) {
                draw_mine(cell_x, cell_y);
            } else if (state->game_phase != m_END &&
                       !bitmatrix_get(state->revealed, cell_x, cell_y)) {
...
__attribute__((optimize("s"))) static void reveal_cell(
    struct MinesGameState *state, uint8_t cell0_x, uint8_t cell0_y) {
                    ...
                    if (mines_count(state, cell_x, cell_y) == 0) {
                        for (int8_t dy = -1; dy <= 1; dy++) {
                            for (int8_t dx = -1; dx <= 1; dx++) {
                                ...
                                if (!bitmatrix_get(state->mines, cell_x + dx,
                                                   cell_y + dy) &&
                                    !bitmatrix_get(to_reveal, cell_x + dx,
                                                   cell_y + dy) &&
                                    !bitmatrix_get(state->revealed, cell_x + dx,
                                                   cell_y + dy)) {
                                    bitmatrix_set(to_reveal, cell_x + dx,
                                                  cell_y + dy);
                                    n_to_reveal++;
                                }
                            }
...
static void boom(struct MinesGameState *state) {
    for (uint8_t i = 0; i < 30; i++) {
        lcd_fillCircleSimple(x_cell(state->cursor_x) + CELL_SIZE / 2,
                             y_cell(state->cursor_y) + CELL_SIZE / 2, i, 1);
        lcd_display();
    }
}
...
void run_mines(void) {
    ...
    struct MinesGameState state = {.mines = mines,
                                   .revealed = revealed,
                                   .flagged = flagged,
    ...
    while (state.game_phase != m_END) {
        ...
        if (key == APP_KEY_SELECT) {
            ...
            if (count > 50 && !bitmatrix_get(state.revealed, state.cursor_x,
                                             state.cursor_y)) {
                if (bitmatrix_get(state.flagged, state.cursor_x,
                                  state.cursor_y)) {
                    bitmatrix_unset(state.flagged, state.cursor_x,
                                    state.cursor_y);
                    state.n_flagged--;
                } else {
                    bitmatrix_set(state.flagged, state.cursor_x,
                                  state.cursor_y);
                    state.n_flagged++;
                }
            } else if (!bitmatrix_get(state.flagged, state.cursor_x,
                                      state.cursor_y)) {
                // Don't reveal flagged cells, this is most likely a misclick.
                if (bitmatrix_get(state.mines, state.cursor_x,
                                  state.cursor_y)) {
                    boom(&state);
                    state.game_phase = m_END;
                } else {
