...
#define norm sqrtf(initial_ball_vx_unscaled * initial_ball_vx_unscaled + initial_ball_vy_unscaled * initial_ball_vy_unscaled)
...
#define ball_speed_increase_factor 1.025f // factor speed increases by each block
...
static void display_ball(Ball *ball) {
    ...
    lcd_display_block(roundf(ball->pos.x) - ball_int_radius, current_tline + 1, 5);
...
static void display_paddle(float paddle_x) {
    ...
    lcd_display_block(xmin < ceil8(paddle_speed) ? 0 : xmin - ceil8(paddle_speed),
                      DISPLAY_HEIGHT / 8 - 1, paddle_width + 1 + 2 * ceil8(paddle_speed));
}
...
void run_breakout() {
    ...
    while (1) {
        ...
        if (key == APP_KEY_LEFT
            ...
            && roundf(state.paddle_x) < DISPLAY_WIDTH - 1 - (paddle_width - 1)/2 - paddle_speed) {
            state.paddle_x += paddle_speed;
