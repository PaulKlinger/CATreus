...
#define initial_ball_speed 1.5f  // speed of ball / frame
#define initial_ball_vx_unscaled 3
#define initial_ball_vy_unscaled -10
#define norm                                                 \
  sqrtf(initial_ball_vx_unscaled *initial_ball_vx_unscaled + \
        initial_ball_vy_unscaled * initial_ball_vy_unscaled)
...
#define ball_speed_increase_factor \
  1.025f  // factor speed increases by each block
...
static void display_ball(Ball *ball) {
  ...
  uint8_t current_tline = (roundf(ball->pos.y) - ball_int_radius) / 8;

  lcd_display_block(roundf(ball->pos.x) - ball_int_radius, current_tline, 5);
  lcd_display_block(roundf(ball->pos.x) - ball_int_radius, current_tline + 1,
                    5);
...
static void draw_ball(Ball *ball) {
  ...
  uint8_t x = roundf(ball->pos.x);
  uint8_t y = roundf(ball->pos.y);
...
static void wall_collision(Ball *ball) {
  if (ball->pos.x < ball_radius) {
    ball->pos.y =
        ball->pos.y - (ball->pos.x - ball_radius) / ball->v.x * ball->v.y;
    ball->pos.x = ball_radius;
    ball->v.x *= -1;
  } else if (ball->pos.x > DISPLAY_WIDTH - ball_radius) {
    ball->pos.y = ball->pos.y - (ball->pos.x - DISPLAY_WIDTH + ball_radius) /
                                    ball->v.x * ball->v.y;
    ball->pos.x = DISPLAY_WIDTH - ball_radius;
    ball->v.x *= -1;
  } else if (ball->pos.y < ball_radius) {
    ball->pos.x =
        ball->pos.x - (ball->pos.y - ball_radius) / ball->v.y * ball->v.x;
...
static struct BlockCoords calc_paddle_coords(float paddle_x) {
  struct BlockCoords ret;
  ret.x1 = roundf(paddle_x) - (paddle_width - 1) / 2;
  ret.y1 = DISPLAY_HEIGHT - 1 - paddle_height;
  ret.x2 = roundf(paddle_x) + (paddle_width - 1) / 2;
...
static void move_ball(Ball *ball) {
  ball->prev_x = roundf(ball->pos.x);
  ball->prev_tline = (roundf(ball->pos.y) - ball_int_radius) / 8;
  ball->pos.x += ball->v.x;
  ball->pos.y += ball->v.y;
}

static void draw_paddle(float paddle_x) {
  uint8_t xmin = roundf(paddle_x) - (paddle_width - 1) / 2;
  lcd_fillRect(xmin, DISPLAY_HEIGHT - 1 - paddle_height, xmin + paddle_width,
               DISPLAY_HEIGHT - 1, 1);
}
static void display_paddle(float paddle_x) {
  uint8_t xmin = roundf(paddle_x) - (paddle_width - 1) / 2;
  lcd_display_block(xmin < ceil8(paddle_speed) ? 0 : xmin - ceil8(paddle_speed),
                    DISPLAY_HEIGHT / 8 - 1,
                    paddle_width + 1 + 2 * ceil8(paddle_speed));
}
...
static BreakoutGamestate init_gamestate() {
  ...
  memset(&state.prev_block_status, 0xFF,
         num_blocks / 8 + (num_blocks % 8 ? 1 : 0));
...
void run_breakout() {
  ...
  while (1) {
    ...
    if (key == APP_KEY_LEFT &&
        roundf(state.paddle_x) > (paddle_width - 1) / 2 + paddle_speed) {
      state.paddle_x -= paddle_speed;
    } else if (key == APP_KEY_RIGHT &&
               roundf(state.paddle_x) <
                   DISPLAY_WIDTH - 1 - (paddle_width - 1) / 2 - paddle_speed) {
      state.paddle_x += paddle_speed;
    ...
    for (uint8_t i = 0; i < num_blocks; i++) {
      if (get_block_status(i, state.block_status) !=
        ...
        if (points % num_blocks == 0) {
          ...
          lcd_display();
          wait_for_wake();
          Vec old_v = state.ball.v;
          state = init_gamestate();
          state.ball.v = old_v;
          while (state.ball.v.y > -2) {
            rotate_vec(&state.ball.v, 1);
          }
          lcd_clrscr();
          draw_blocks(state.block_status);
          lcd_display();
