...
const struct gpio_dt_spec wake_btn =
    GPIO_DT_SPEC_GET(DT_NODELABEL(wakebtn), gpios);

static struct gpio_callback button_cb_data;
gpio_port_pins_t detect_pins = BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(4);

gpio_port_pins_t drive_pins = BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(4) |
...
void init_key_matrix(void) {
  ...
  for (int i = 0; i < ARRAY_SIZE(gpio_rows); i++) {
    gpio_pin_configure_dt(&gpio_rows[i],
...
bool wake_pressed(void) { return gpio_pin_get_dt(&wake_btn); }
